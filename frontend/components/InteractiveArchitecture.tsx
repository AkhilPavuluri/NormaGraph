...
const StorageNode = ({ data, selected }: { data: any; selected: boolean }) => (
  <motion.div
    initial={{ scale: 0.8, opacity: 0 }}
    animate={{ scale: 1, opacity: 1 }}
    className={`px-4 py-3 rounded-lg border-2 bg-gradient-to-br from-slate-50 to-slate-100 border-slate-300 dark:from-slate-950/40 dark:to-slate-900/50 dark:border-slate-600 min-w-[200px] ${
      selected ? 'ring-2 ring-primary' : ''
    }`}
  >
    <Handle type="target" position={Position.Top} />
    <div className="flex items-center gap-2 mb-2">
      <data.icon className="h-5 w-5 text-slate-600 dark:text-slate-300" />
      <span className="font-semibold text-slate-800 dark:text-slate-100">{data.label}</span>
    </div>
...
const OutputNode = ({ data, selected }: { data: any; selected: boolean }) => (
  <motion.div
    initial={{ scale: 0.8, opacity: 0 }}
    animate={{ scale: 1, opacity: 1 }}
    className={`px-4 py-3 rounded-lg border-2 bg-gradient-to-br from-primary/10 to-primary/5 border-primary/30 min-w-[200px] ${
      selected ? 'ring-2 ring-primary' : ''
    }`}
  >
    <Handle type="target" position={Position.Top} />
      ...
      <div className="absolute bottom-4 left-4 z-10">
        <Card className="w-64">
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-slate-500 dark:bg-slate-400"></div>
